...
# FIX 1: Refined prompt — no follow-up questions, answer directly and completely
CUSTOM_PROMPT_TEMPLATE = """
You are MediBot, a medical information assistant powered by the Gale Encyclopedia of Medicine.

STRICT RULES — FOLLOW WITHOUT EXCEPTION:
1. NEVER ask the user any questions. Not even one. Ever.
2. NEVER ask about symptoms, age, diagnosis, or any personal details.
3. ALWAYS give a complete, informative answer immediately.
4. If the question is general (e.g. "tell me about cancer"), give a full general overview.

Your answer must include (where relevant):
- Definition / Overview
- Causes
- Symptoms
- Treatments
...
If context has limited info, use what is available.
If you truly don't know, say: "I don't have enough information on this topic."
End every response with: "⚕️ Always consult a qualified doctor for personal medical advice."

Context: {context}
Question: {question}

Provide a complete informative answer below. DO NOT ask any questions:
"""

# FIX 2: How many past exchanges (user+assistant pairs) to keep in history
HISTORY_WINDOW = 3  # last 3 exchanges = last 6 messages
...
# FIX 3: Build limited chat history — only last N exchanges to avoid context overflow
def build_history_text(messages):
    """
    Takes the full session messages list (excluding the current/latest message),
    keeps only the last HISTORY_WINDOW exchanges (user+assistant pairs),
    strips source content, and returns a formatted string.
    """
    # Exclude the last message (current user prompt already handled separately)
    past_messages = messages[:-1]

    # Keep only the last (HISTORY_WINDOW * 2) messages = N user+assistant pairs
    windowed = past_messages[-(HISTORY_WINDOW * 2):]

    history_text = ""
    for msg in windowed:
        role = "User" if msg['role'] == 'user' else "MediBot"
        # Strip sources from assistant messages before adding to history
    ...
    with st.chat_message("assistant", avatar="🩺"):
        with st.spinner("Searching the medical encyclopedia…"):
            try:
                ...
                # FIX 3 applied: build limited, clean history
                history_text = build_history_text(st.session_state.messages)

                # Compose the full query with history context (if any)
                if history_text:
                    full_query = f"""Previous conversation (last {HISTORY_WINDOW} exchanges for context):
{history_text}

Current question: {prompt}

Note: Use the conversation history only as context to better understand the current question. Answer the current question directly and completely."""
                else:
                    full_query = prompt
